...
function BankSampahContent() {
  ...
  const [activeTab, setActiveTab] = useState<
    'master' | 'iuran' | 'penjualan' | 'operasional' | 'rekap'
  >('master');
  ...
  // Specific flag for Banyubiru desa where we need to hide add/actions
  const isBanyubiru = useMemo(() => {
    return Boolean(
      currentDesa &&
      typeof currentDesa.nama === 'string' &&
  ...
  const [dusunModalMode, setDusunModalMode] = useState<
    'create' | 'edit' | 'delete' | null
  >(null);
  ...
  useEffect(() => {
    ...
    async function fetchDesa() {
      try {
        ...
        if (meData.ok) {
          setIsAdmin(
            Boolean(meData.role === 'admin' || meData.role === 'superadmin'),
          );
          if (meData.desaId) {
    ...
    async (targetDesa = selectedDesaId) => {
      ...
      try {
        ...
        const [memberRes, wilayahRes] = await Promise.all([
          fetch(`/api/member-bank-sampah?${params.toString()}`, {
            cache: 'no-store',
          }),
  ...
  const kpiMember = useMemo(() => {
    ...
    const rumahan = members.filter(
      (m) => (m.kategori || 'Rumahan') === 'Rumahan',
    ).length;
  ...
  const kpiDusun = useMemo(() => {
    ...
    const totalKK = wilayahList.reduce(
      (acc, w) => acc + Number(w.jumlah_kk || 0),
      0,
    );
    const totalJiwa = wilayahList.reduce(
      (acc, w) => acc + Number(w.jumlah_jiwa || 0),
      0,
    );
  ...
  const filteredMembers = useMemo(() => {
    return members.filter((m) => {
      ...
      const matchKategori =
        !kategoriFilter || (m.kategori || 'Rumahan') === kategoriFilter;
  ...
  async function handleMemberSubmit(e: FormEvent) {
    ...
    try {
      ...
      showErrorToast(
        err instanceof Error ? err.message : 'Gagal menyimpan data member.',
      );
    } finally {
  ...
  async function handleDusunSubmit(e: FormEvent) {
    ...
    try {
      const payload = {
        ...
        jumlah_jiwa: dusunForm.jumlah_jiwa
          ? parseInt(dusunForm.jumlah_jiwa, 10)
          : 0,
      ...
      showErrorToast(
        err instanceof Error ? err.message : 'Gagal menyimpan data dusun.',
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  async function handleDusunDeleteConfirm() {
    ...
    try {
      ...
      showErrorToast(
        err instanceof Error ? err.message : 'Gagal menghapus dusun.',
      );
    } finally {
      setIsSubmitting(false);
    }
      ...
      >
        ...
        <div className="page-header-clean">
          ...
          <div className="header-actions-clean">
            ...
            {activeTab === 'master' && !(!isDesaDukun && isBanyubiru) && (
              <button
                type="button"
                className="btn-primary-clean"
                onClick={
                  isDesaDukun ? openCreateMemberModal : openCreateDusunModal
                }
              >
                <Plus size={15} />
                <span>
              ...
              <div className="kpi-grid-clean">
                <div className="kpi-card-clean">
                  ...
                  <div className="kpi-text-clean">
                    ...
                    <span className="kpi-value-clean">
                      {kpiMember.total}{' '}
                      <span className="kpi-unit-clean">orang</span>
                    </span>
                  </div>
                </div>
                ...
                <div className="kpi-card-clean">
                  ...
                  <div className="kpi-text-clean">
                    ...
                    <span className="kpi-value-clean">
                      {kpiMember.rumahan}{' '}
                      <span className="kpi-unit-clean">warga</span>
                    </span>
                  </div>
                </div>
                ...
                <div className="kpi-card-clean">
                  ...
                  <div className="kpi-text-clean">
                    ...
                    <span className="kpi-value-clean">
                      {kpiMember.industri}{' '}
                      <span className="kpi-unit-clean">usaha</span>
                    </span>
                  </div>
                </div>
                ...
                <div className="kpi-card-clean">
                  ...
                  <div className="kpi-text-clean">
                    <span className="kpi-label-clean">
                      Wilayah / Dusun Terlayani
                    </span>
                    <span className="kpi-value-clean">
                      {kpiMember.wilayahTerlayani}{' '}
                      <span className="kpi-unit-clean">dusun</span>
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="form-group-clean">
                  ...
                  >
                    <button
                      type="button"
                      onClick={() =>
                        setMemberForm({ ...memberForm, kategori: 'Rumahan' })
                      }
                      style={{
                        padding: '12px',
                        borderRadius: '10px',
                        border:
                          memberForm.kategori === 'Rumahan'
                            ? '2px solid #059669'
                            : '1px solid #e2e8f0',
                        background:
                          memberForm.kategori === 'Rumahan'
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setMemberForm({ ...memberForm, kategori: 'Industri' })
                      }
                      style={{
                        padding: '12px',
                        borderRadius: '10px',
                        border:
                          memberForm.kategori === 'Industri'
                            ? '2px solid #2563eb'
                            : '1px solid #e2e8f0',
                        background:
                          memberForm.kategori === 'Industri'
                ...
                >
                  <div className="form-group-clean">
                    ...
                    <input
                      ...
                      placeholder={
                        memberForm.kategori === 'Industri'
                          ? 'Contoh: Warung Berkah / Toko Sejahtera'
                          : 'Contoh: Budi Santoso'
                      }
          ...
          <div className="modal-backdrop-clean">
            <div className="modal-dialog-clean modal-dialog-sm-clean">
              ...
              >
                ...
                >
                  Catatan: Data transaksi sampah yang sudah tersimpan tetap aman
                  di database.
                </small>
              </div>
              <div
