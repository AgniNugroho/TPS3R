...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ ok: true, path, publicUrl: data.publicUrl }, { status: 201 });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Gagal mengunggah file ke Supabase Storage.";
