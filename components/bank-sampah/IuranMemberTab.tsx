...
export default function IuranMemberTab({ selectedDesaId, members }: Props) {
    ...
    const kpi = useMemo(() => {
        ...
        const unpaidCount = members.filter((m) => m.status === "Aktif" && !paidMemberIds.has(m.id)).length;
        ...
        return {
            ...
            lunasCount,
            unpaidCount,
            totalMemberAktif: members.filter((m) => m.status === "Aktif").length,
        };
    }, [payments, members]);
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            if (modalMode === "create") {
                ...
                showSuccessToast("Pembayaran iuran member berhasil dicatat!");
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
                    ...
                    <span style={{ fontSize: "12px", color: "#62736d", background: "#f0fdf4", padding: "4px 10px", borderRadius: "100px", border: "1px solid #bbf7d0" }}>
                        💡 Pembayaran iuran rutin member dilakukan tgl <strong>1–7 awal bulan</strong>.
                    </span>
                </div>
            ...
            <div className="kpi-grid-clean">
                ...
                <div className="kpi-card-clean">
                    ...
                    <div className="kpi-text-clean">
                        <span className="kpi-label-clean">Member Sudah Bayar</span>
                        <span className="kpi-value-clean">
                            {kpi.lunasCount} <span className="kpi-unit-clean">/ {kpi.totalMemberAktif} member</span>
                        </span>
                    </div>
                </div>
                ...
                <div className="kpi-card-clean">
                    ...
                    <div className="kpi-text-clean">
                        <span className="kpi-label-clean">Member Belum Bayar</span>
                        <span className="kpi-value-clean" style={{ color: kpi.unpaidCount > 0 ? "#b45309" : "var(--teal)" }}>
                            {kpi.unpaidCount} <span className="kpi-unit-clean">orang</span>
                        </span>
                    </div>
                </div>

                <div className="kpi-card-clean">
                    <div className="kpi-icon-wrap-clean" style={{ background: "#e0e7ff", color: "#4338ca" }}>
                        <Banknote size={20} />
                    </div>
                    <div className="kpi-text-clean">
                        <span className="kpi-label-clean">Metode Pembayaran</span>
                        <span className="kpi-value-clean" style={{ fontSize: "13px", fontWeight: 700 }}>
                            Cash: Rp {kpi.cashTotal.toLocaleString("id-ID")}<br />
            ...
            <div style={{
                ...
                <div style={{ position: "relative", flex: 1, minWidth: "220px" }}>
                    ...
                    <input
                        type="text"
                        placeholder="Cari nama member, dusun, atau catatan..."
                        value={searchQuery}
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                        <thead>
                            <tr style={{ background: "#f8faf9", borderBottom: "1px solid var(--line)", textAlign: "left" }}>
                                ...
                                <th style={{ padding: "14px 16px", fontWeight: 700, color: "#475569" }}>Member</th>
                                <th style={{ padding: "14px 16px", fontWeight: 700, color: "#475569" }}>Dusun / Wilayah</th>
                ...
                <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
                    <div style={{ background: "white", borderRadius: "20px", maxWidth: "500px", width: "100%", padding: "28px", boxShadow: "0 20px 40px rgba(0,0,0,0.2)" }}>
                        <h3 style={{ margin: "0 0 20px 0", fontSize: "18px", fontWeight: 800, color: "#1a2522" }}>
                            {modalMode === "create" ? "Catat Pembayaran Iuran Member" : "Edit Pembayaran Member"}
                        </h3>
                        ...
                        <form onSubmit={handleSubmit} style={{ display: "grid", gap: "16px" }}>
                            {modalMode === "create" ? (
                                ...
                                <div style={{ background: "#f8faf9", padding: "12px", borderRadius: "8px", border: "1px solid var(--line)" }}>
                                    ...
                                    <div style={{ fontWeight: 700, fontSize: "14px", color: "#1a2522" }}>{editItem?.member?.nama}</div>
                                </div>
                            )}
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    style={{ width: "100%", padding: "10px 12px", borderRadius: "8px", border: "1px solid var(--line)", fontSize: "14px", fontWeight: 700, outline: "none", background: "#fbfdfb" }}
                                />
                                <div style={{ display: "flex", gap: "6px", marginTop: "6px" }}>
                                    {["15000", "20000", "25000", "50000"].map((nominalPreset) => (
                                        <button
                                            key={nominalPreset}
                                            type="button"
                                            onClick={() => setForm({ ...form, nominal: nominalPreset })}
                                            style={{
                                                padding: "3px 8px",
                                                borderRadius: "4px",
                                                fontSize: "11px",
                                                border: "1px solid #cbd5e1",
                                                background: form.nominal === nominalPreset ? "var(--teal)" : "#f8faf9",
                                                color: form.nominal === nominalPreset ? "white" : "#475569",
                                                cursor: "pointer"
                                            }}
                                        >
                                ...
                                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                                    <button
                                        ...
                                        style={{
                                            ...
                                            background: form.metode_pembayaran === "Cash" ? "#f0fdf4" : "white",
                ...
                <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
                    <div style={{ background: "white", borderRadius: "18px", maxWidth: "420px", width: "100%", padding: "24px" }}>
                        ...
                        <p style={{ margin: "0 0 20px 0", fontSize: "13px", color: "#64748b", lineHeight: 1.5 }}>
                            Anda yakin ingin menghapus data pembayaran <strong>{deleteItem.member?.nama}</strong> sebesar <strong>Rp {Number(deleteItem.nominal || 0).toLocaleString("id-ID")}</strong> pada periode {deleteItem.periode_bulan}?
                        </p>
                        <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px" }}>
                            ...
                            <button
                                ...
                                style={{ background: "#dc2626", color: "white", border: "none", padding: "8px 16px", borderRadius: "8px", fontSize: "13px", fontWeight: 700, cursor: "pointer" }}
                            >
