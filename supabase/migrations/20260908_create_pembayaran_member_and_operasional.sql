...
-- 1. Table for member / dusun monthly fee payments
create table if not exists public.pembayaran_member (
  ...
  member_id uuid references public.member_bank_sampah(id) on delete cascade,
  wilayah_id uuid references public.wilayah(id) on delete cascade,
...
-- 3. Indexes for query performance & uniqueness
...
create unique index if not exists uq_member_periode on public.pembayaran_member(member_id, periode_bulan) where member_id is not null;
